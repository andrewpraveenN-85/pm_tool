...
// Get initial tasks for modal - all active tasks
...
if (isset($_GET['edit_bug'])) {
    ...
    if ($edit_bug) {
        ...
        // Get tasks for edit modal for the current project
        $edit_task_list = $db->prepare("
            SELECT t.id, t.name 
            FROM tasks t 
            WHERE t.project_id = ? 
            AND t.status != 'closed' 
            ...
            ORDER BY t.name
        ");
        $edit_task_list->execute([$edit_bug['project_id']]);
        $edit_tasks_for_modal = $edit_task_list->fetchAll(PDO::FETCH_ASSOC);
    }
}
?>
<!DOCTYPE html>
...
<body>
                ...
                <form method="POST" enctype="multipart/form-data" id="createBugForm">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control wysiwyg" name="description" id="bug_description" rows="4"><?= isset($_POST['description']) ? htmlspecialchars($_POST['description']) : '' ?></textarea>
                ...
                <?php if ($edit_bug): ?>
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div class="position-relative">
                                        <select class="form-select" name="task_id" id="editTaskSelect" required>
                                            <option value="">Select Task</option>
                                            <?php if (isset($edit_tasks_for_modal)): ?>
                                                <?php foreach ($edit_tasks_for_modal as $task): ?>
                            ...
                            <div class="mb-3">
                                <label class="form-label required">Description</label>
                                <textarea class="form-control wysiwyg" name="description" rows="4"><?= htmlspecialchars($edit_bug['description']) ?></textarea>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Initialize TinyMCE with proper configuration to avoid validation issues
            tinymce.init({
                ...
                init_instance_callback: function(editor) {
                    // Ensure the original textarea is updated when form is submitted
            ...
            // Handle project change for task filtering
            $('#projectSelect').on('change', function() {
                updateTaskDropdown();
            });

            // Handle edit project change for task filtering
            ...
            // Fix form validation for TinyMCE
            $('#createBugForm, #editBugModal form').on('submit', function(e) {
                // Trigger TinyMCE to save content to textarea
                tinymce.triggerSave();
                
                // Manually validate TinyMCE content
                const descriptionField = $(this).find('textarea.wysiwyg');
                const content = tinymce.get(descriptionField.attr('id')).getContent();
        ...
        function updateTaskDropdown() {
            ...
            if (projectId) {
                    ...
                    .then(data => {
                        if (data && data.length > 0) {
                            data.forEach(task => {
                                ...
                                optionEl.textContent = task.name;
                                taskSelect.appendChild(optionEl);
                            });
                        } else {
                            const optionEl = document.createElement('option');
                            optionEl.value = '';
                            optionEl.textContent = 'No tasks available for this project';
                            taskSelect.appendChild(optionEl);
                        }
                    })
                    .catch(error => {
                        ...
                        const optionEl = document.createElement('option');
                        optionEl.value = '';
                        optionEl.textContent = 'Error loading tasks. Please try again.';
                        taskSelect.appendChild(optionEl);
                    })
                    .finally(() => {
                ...
                <?php foreach ($initial_tasks as $task): ?>
                    const optionEl = document.createElement('option');
                    optionEl.value = '<?= $task['id'] ?>';
                    optionEl.textContent = '<?= addslashes($task['name']) ?>';
                    taskSelect.appendChild(optionEl);
                <?php endforeach; ?>
            }
        }
        ...
        function updateEditTaskDropdown() {
            ...
            if (projectId) {
                ...
                foreach ($all_edit_tasks as $task): ?>
                    ...
                    // Preselect the current task
                    if (<?= $edit_bug ? $edit_bug['task_id'] : 'null' ?> && <?= $task['id'] ?> == <?= $edit_bug ? $edit_bug['task_id'] : 'null' ?>) {
                        optionEl.selected = true;
                    }
